...
> *Definition.*{: .def}
> The set of all [permutations](permutations.md) of $X$ is denoted by $\text{Sym} X$ or $\mathcal{P}(X)$.
...
> *Definition.*{: .def}
> If $X$ is finite, say $\vert X \vert = n$, we usually use $X = \set{1, 2, ..., n}$ and $\text{Sym} X = S_n$, which is called a **symmetric group**.
...
>
> Also, $A\_n = \ker \text{sgn}$ which [is a subgroup](homomorphisms.md#kernel-is-subgroup) of $S\_n$.
